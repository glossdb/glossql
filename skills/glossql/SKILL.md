...
| `GLOSS aspect ON subject AS $$json$$;` | speak a value into your slot; an aspect ON TABLE or ON COLUMN takes only a landed table or column; a QUERY aspect is grounded on the dataset (`GLOSS revenue ON fin AS …`), and the outcome is the metric's fact row (the `metric_axes()` shape, above) |
