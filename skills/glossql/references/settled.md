...
| the app's series and slices | `metric_series(grain => …)` — the cube: one head per metric at the floor grain, landed under the cache, every grain a plan over it; `metric_axes()` says what it admitted, what the window leaves out, and, per served column, what keeps the rest out |
