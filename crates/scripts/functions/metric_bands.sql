...
-- withholds the PIT, and the detector scores the month before it. The
-- band_breach detector adjudicates PITs; this measurement only
-- reports.
--
...
-- a choice of column and never of grain. `aggregation` names what one
-- month's value is: `sum` for a flow, `latest-sum` for a stock,
-- `ratio-of-sums` for a ratio.
WITH m AS (
  SELECT seq, metric, applicable, reason, grain, aggregation, trained_on,
         ...
         CASE WHEN applicable THEN coalesce(array_agg(named_struct(
           ...
           'pit', pit, 'withheld', withheld, 'partial', partial
