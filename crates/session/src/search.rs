...
pub(crate) async fn derivation_candidates(
    ...
    if specs.is_empty() {
        return rows_batch(vec![fact(Default::default())]);
    }
...
/// The door's fixed shape, decoded from JSON rows through the format's
/// own decoder — the same trick the profile aggregate uses, so there is
/// no hand-built array assembly to drift.
fn rows_batch(rows: Vec<Value>) -> Result<RecordBatch, SessionError> {
