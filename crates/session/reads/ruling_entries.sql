...
--
-- Flat on purpose. Lifting the newest-writing filter into a CTE that
-- projects `body` breaks datafusion-functions-json's rewrite — the
-- outer json_get calls resolve against `__datafusion_extracted_*`
-- columns and planning fails with "No field named c.body". Keep the
-- json accessors in the same SELECT as the scan they read from.
