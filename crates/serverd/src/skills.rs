...
//! files on demand). The normative artifacts and the `docs/` pages
//! ride along as `doc://` resources, because a connected agent has no
//! checkout to read them from — what an agent in this repository
//! reads is what the door serves.
...
/// `SKILL.md`, `PAGES` every `.md` under `docs/`.
pub struct Page {
    pub path: &'static str,
    pub body: &'static str,
}
...
impl Page {
    ...
    /// `SKILL.md` has; a docs page is `doc://<repo path>`, the same
    /// root as `doc://SPEC.md`.
    pub fn uri(&self) -> String {
