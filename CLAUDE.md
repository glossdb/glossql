...
- `.claude/skills/` — where a door-connected agent learns the
  language, plus the substrate skill for building the server; every
  skill is gated by the test suite.
...
- **Standing invariant.** Workspace `cargo test` passes: every
  ...
  tagged, every fenced example under `docs/` and `.claude/skills/`
  parses and plans, and the store and session suites hold the
  execution semantics. A change that breaks it doesn't land.
